...
namespace PSEBONLINE.AbstractLayer
{
	public class AssociateDB
	{
		...
		public DataSet GetMeritoriousAdmissionDataByStudentRollNoAPI(string roll, string type)
		{
			...
			try
			{
				using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings[CommonCon].ToString()))
				{
					connection.Open();

					using (SqlCommand cmd = new SqlCommand("GetMeritoriousAdmissionDataByStudentRollNoAPI", connection))
					{
						cmd.CommandType = CommandType.StoredProcedure;
		...
		public DataSet SearchUdicCodeByApi(string UdicCode)
		{
			...
			try
			{
				using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings[CommonCon].ToString()))
				{
					...
					using (SqlCommand cmd = new SqlCommand("GetEpschoolmasterCenterHeadNoAPI", connection))
					{
						cmd.CommandType = CommandType.StoredProcedure;
						cmd.Parameters.AddWithValue("@Udisecode", UdicCode);
						using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
						{
							adapter.Fill(resultDataSet);
						}
